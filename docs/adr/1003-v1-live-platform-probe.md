...
5. Test unauthorized WebSocket rejection before authorized join. The channel remains restricted to `platform-probe:<uuid>` actor keys.
...
- Senior 5 must provide `LIQI_PROBE_AUTH_TOKEN_REF` (preferred) or the compatibility `LIQI_PROBE_AUTH_TOKEN`, and register the exact provider commit.
...
```bash
python -m unittest beam.tests.test_platform_probe -v
