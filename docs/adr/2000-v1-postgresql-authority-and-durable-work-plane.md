...
13. `LiqiPersistence.RuntimeAdapter` implements the persistence callback consumed by Senior 1 at `v1/beam-runtime-realtime@96ce092e0381225e840e70ce5c81e076f6b8499a`, including the canonical `observe_probe/2` seam. Senior 1 remains owner of event identity through the command module's `event_id/1`, Repo supervision, and runtime admission. The provider converts the runtime envelope's Unix epoch millisecond deadline to PostgreSQL `timestamptz`; expired commands are rejected before a database transaction is opened.
14. The fail-closed `Liqi.Persistence.PostgresV1` placeholder on Senior 1's branch is temporary. Removal condition: integrate this provider commit into the root Mix project, configure `LiqiPersistence.Repos` to the three Senior 1 Repo modules, pass provider/consumer integration tests, and replace `:persistence_adapter` with `LiqiPersistence.RuntimeAdapter`. Senior 1 owns removal of the placeholder; Senior 2 owns keeping the callable database seam compatible.
