...
5. PostgreSQL V1 is the only production persistence path. `LiqiPersistence.RuntimeAdapter` consumes migration-8 functions through three root Repo modules and one role URL bundle. The V0 adapter is route-scoped rollback compatibility only and is never the V1 default.
