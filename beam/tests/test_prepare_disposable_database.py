...
def admin_dsn(host="127.0.0.1", port=5432, database="liqi_v1_ci", credential=None, query=""):
    userinfo = "postgres" + ((":" + quote(credential, safe="")) if credential else "")
    netloc = userinfo + "@" + host + ((":" + str(port)) if port else "")
    return urlunsplit(("postgres", netloc, "/" + database, query, ""))

class PrepareDisposableDatabaseTest(unittest.TestCase):
    def test_writes_role_bundle(self):
        with tempfile.TemporaryDirectory() as directory:
            result=write_inputs(admin_dsn(),Path(directory),'liqi_v1_test_runtime')
    ...
    def test_rejects_unsafe_admin_urls(self):
        for value in [admin_dsn(host='db.internal'), admin_dsn(credential='test-value'), admin_dsn(query='x=1'), admin_dsn(database='liqi')]:
            with self.subTest(value=value),self.assertRaises(ValueError): parse_admin_url(value)
    def test_rejects_unsafe_target(self):
        with tempfile.TemporaryDirectory() as directory,self.assertRaises(ValueError): write_inputs(admin_dsn(port=None),Path(directory),'liqi')
