...
class ProviderCapacityTests(unittest.TestCase):
 ...
 def test_all_provider_budgets_aggregate(self):
  r,p=self.invoke(FIX/'available.json');self.assertEqual(r.returncode,0,r.stderr);self.assertEqual(p['status'],'passed');self.assertLessEqual(p['steady_state_totals']['ocpu'],p['envelope']['steady_state_limit']['ocpu']);self.assertLessEqual(p['hard_limit_totals']['ocpu'],p['envelope']['provider_hard_limit']['ocpu']);self.assertLessEqual(p['hard_limit_totals']['memory_mib'],p['envelope']['provider_hard_limit']['memory_mib']);self.assertLessEqual(p['postgres_connection_accounting']['pooled_runtime_demand'],p['postgres_connection_accounting']['pooled_server_capacity'])
