...
doc={"schema_version":"liqi.infrastructure.plan-result/v1","environment":"v1-live","git_sha":git_sha,"mode":mode,"capacity_profile":capacity_profile,"plan_mode":plan_mode,"approval_reference":approval or None,"created_at":datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),"saved_plan":{"path":plan,"sha256":sha(plan)},"plan_json":{"path":plan_json,"sha256":sha(plan_json)},"validation":{"path":validation,"sha256":sha(validation),"status":"passed"},"inputs":{"state_backend_evidence_sha256":sha(state_evidence),"adoption_result_sha256":sha(adoption_result) if adoption_result else None,"pre_apply_readiness_sha256":sha(pre_apply_readiness) if pre_apply_readiness else None,"var_file_sha256":sha(var_file),"adoption_manifest_sha256":readiness_inputs.get("adoption_manifest_sha256"),"linux_release_build_result_sha256":readiness_inputs.get("linux_release_build_result_sha256"),"recovery_target_sha256":readiness_inputs.get("recovery_target_sha256")},"state_backend":{"kind":"postgresql-self-hosted","schema":"opentofu_v1_live","tls":"verify-full"},"tf_data_dir":tf_data,"oci_mutation_performed":False}
