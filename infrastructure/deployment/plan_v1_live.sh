...
umask 077
usage(){ cat >&2 <<'EOF'
Usage: plan_v1_live.sh --state-backend-evidence FILE --var-file FILE --output-dir DIR [--approved-apply-plan --approval-reference REF] [--allow-reserved-ip]
...
EOF
exit 64; }
state_evidence=''; var_file=''; output_dir=''; mode=plan; approval_reference=''; allow_reserved_ip=false
while (($#)); do case "$1" in
...
root=$(git rev-parse --show-toplevel); cd "$root"; git diff --quiet && git diff --cached --quiet || { echo 'tracked changes are forbidden for an exact-SHA live plan' >&2; exit 66; }
...
args=(-input=false -lock=true -lock-timeout=60s -out="$plan_file" -var-file="$var_file" -var="source_git_sha=$git_sha" -var="operation_mode=$mode" -var="apply_approval_reference=$approval_reference")
...
vargs=("$plan_json" --mode "$mode" --output "$validation"); [[ "$allow_reserved_ip" == true ]] && vargs+=(--allow-reserved-ip); python "$validator" "${vargs[@]}" >/dev/null
python - "$evidence" "$git_sha" "$mode" "$approval_reference" "$plan_file" "$plan_json" "$validation" "$state_evidence" "$var_file" "$TF_DATA_DIR" <<'PY'
import hashlib,json,sys
from datetime import datetime,timezone
from pathlib import Path
def sha(p): return hashlib.sha256(Path(p).read_bytes()).hexdigest()
out,git_sha,mode,approval,plan,plan_json,validation,state_evidence,var_file,tf_data=sys.argv[1:]
doc={"schema_version":"liqi.infrastructure.plan-result/v1","environment":"v1-live","git_sha":git_sha,"mode":mode,"approval_reference":approval or None,"created_at":datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),"saved_plan":{"path":plan,"sha256":sha(plan)},"plan_json":{"path":plan_json,"sha256":sha(plan_json)},"validation":{"path":validation,"sha256":sha(validation),"status":"passed"},"inputs":{"state_backend_evidence_sha256":sha(state_evidence),"var_file_sha256":sha(var_file)},"state_backend":{"kind":"postgresql-self-hosted","schema":"opentofu_v1_live","tls":"verify-full"},"tf_data_dir":tf_data,"oci_mutation_performed":False}
