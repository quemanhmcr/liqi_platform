...
validate_common; require_command tofu; require_command python; require_command psql
...
[ "$lock_schema" != "$STATE_SCHEMA" ] || fail 'lock test schema must never be the live state schema'
admin_service_file=$PGSERVICEFILE
PGSERVICEFILE="$admin_service_file" psql "service=$STATE_ADMIN_SERVICE dbname=$STATE_DATABASE" -v ON_ERROR_STOP=1 -v s="$lock_schema" -v r="$STATE_ROLE" <<'SQL' >/dev/null
SELECT format('DROP SCHEMA IF EXISTS %I CASCADE',:'s') \gexec
...
SQL
export PG_SCHEMA_NAME=$lock_schema
export PG_SKIP_SCHEMA_CREATION=true PG_SKIP_TABLE_CREATION=false PG_SKIP_INDEX_CREATION=false
...
cleanup(){
  ...
  PGSERVICEFILE="$admin_service_file" psql "service=$STATE_ADMIN_SERVICE dbname=$STATE_DATABASE" -v ON_ERROR_STOP=1 -v s="$lock_schema" <<'SQL' >/dev/null 2>&1 || true
SELECT format('DROP SCHEMA IF EXISTS %I CASCADE',:'s') \gexec
SQL
}; trap cleanup EXIT
